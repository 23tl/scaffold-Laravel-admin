...
<div class="layui-form layuimini-form">
        ...
        <div class="layui-input-block">
            <select name="type" lay-verify="required" lay-reqtext="请选择广告位置">
                ...
                @foreach(\App\Models\Banner::$typeMap as $str => $value)
                    <option value="{{ $str }}" @if($str === $banner->type) selected="selected" @endif>{{ $value }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="name" lay-verify="required" lay-reqtext="名称不能为空" placeholder="请输入名称" value="{{ old('name', $banner->name) }}" class="layui-input">
        </div>
    </div>
        ...
        <div class="layui-input-block">
            <select name="urlType" lay-verify="required" lay-reqtext="请选择跳转类型">
                ...
                @foreach(\App\Models\Banner::$urlTypeMap as $str => $value)
                    <option value="{{ $str }}" @if($banner->urlType === $str) selected="selected" @endif>{{ $value }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="url" lay-verify="required" lay-reqtext="链接不能为空" placeholder="请输入链接" value="{{ old('url', $banner->url) }}" class="layui-input">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="sort" lay-verify="required" lay-reqtext="链接不能为空" placeholder="请输入链接" value="{{ old('sort', $banner->sort) }}" class="layui-input">
        </div>
    </div>
        ...
        <div class="layui-input-block">
            <div class="layui-upload-drag" id="test10">
                ...
                <div class="layui-show" id="uploadDemoView">
                    <hr>
                    <img src="{{ getFile($banner->image) }}" alt="上传成功后渲染" style="max-width: 196px;">

                </div>
            </div>
        </div>

        <input type="hidden" value="{{ $banner->image }}" id="cover" name="image">
...
<script>
    ...
    layui.use(['form', 'upload'], function () {
        ...
        form.on('submit(saveBtn)', function (data) {
            postAjaxDestroy('{{ route('admin.banner.update') }}', data.field, iframeIndex);
            return false;
        });
